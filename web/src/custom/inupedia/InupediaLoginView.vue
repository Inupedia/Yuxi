<template>
  <div class="login-view" :class="{ 'has-alert': serverStatus === 'error' }">
      ...
      <div class="login-card">
        ...
        <div class="card-side is-image">
          ...
          <div class="brand-visual-content">
            <span class="visual-kicker">YDYM WATER RESOURCES</span>
            <h2>让工程知识<br />贯通引调水建设全线</h2>
            <p>连接建设资料、智能体与协同工作流，让项目经验沉淀为持续可用的组织能力。</p>
            <div class="visual-capabilities" aria-label="平台能力">
              <span>工程知识</span>
              <span>智能协同</span>
              <span>可信检索</span>
            </div>
        ...
        <div class="card-side is-form">
          <div class="form-wrapper">
            <header class="form-header">
              ...
              <template v-else>
                <p class="welcome-kicker">引大济岷工作台</p>
                <h2 class="welcome-text">欢迎回来</h2>
                <span class="welcome-description">登录后继续访问智能知识与工程协同平台</span>
              </template>
            </header>
    ...
    <footer class="page-footer">
      <div class="copyright">
        &copy; {{ new Date().getFullYear() }} {{ brandOrgName || brandName }}
      </div>
    </footer>
  </div>
</template>
...
const brandName = computed(() => {
  ...
  const brandNameRaw = infoStore.branding?.name?.trim() || '引大济岷'
...
.login-view {
  ...
  background:
    radial-gradient(circle at 10% 10%, var(--main-50), transparent 30%),
    radial-gradient(circle at 92% 86%, var(--brand-green-soft), transparent 28%), var(--gray-25);
...
.login-card {
  ...
  border-radius: 22px;
  box-shadow: 0 26px 70px rgba(23, 36, 44, 0.11);
  display: flex;
  overflow: hidden;
...
.card-side.is-image {
  ...
  background: linear-gradient(
    150deg,
    color-mix(in srgb, var(--main-50) 88%, var(--gray-0)),
    var(--gray-25)
  );
  overflow: hidden;
...
.card-side.is-image::after {
  ...
  background: linear-gradient(
    145deg,
    transparent 24%,
    color-mix(in srgb, var(--main-100) 30%, transparent)
  );
  pointer-events: none;
}
...
.visual-capabilities span {
  ...
  border-radius: 999px;
  background: color-mix(in srgb, var(--gray-0) 66%, transparent);
  color: var(--main-800);
  font-size: 12px;
  font-weight: 550;
}
